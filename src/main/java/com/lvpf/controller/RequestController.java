...
public class RequestController {
	 ...
	 * 当发送请求的时候，找到对应的处理方法之后，会根据参数的名称从request中获取对应的参数值并封装到方法中，
	 * 此时要求方法的名字和url中参数的名字必须一致，如果不一致，设置不成功
	 *
	 * 如果设置的值不同，同时又想让参数获取到对应的属性值可以通过@RequestParam来使用  ？后面key=value
	 * 经常跟@PathVariable混淆，需要注意，两个注解有不同的用处路径{}/{}
	 * 此注解的参数：
	 *          value：获取的参数值
	 *          required：表示当前属性值是否是必须存在的，默认值为true表示请求中必须要包含此参数，如果没有，400，bad request
	 *          defaultValue：如果传递参数那么使用传递进来的参数，如果没有使用默认值
	 * @param name
	 * @return
	 */
